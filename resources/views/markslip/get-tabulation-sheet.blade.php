                      ...
                      @foreach($student->remarks as $remarks)
                        @switch($remarks->type)
                          @case('grade')
                            <td>
                              @if(!$remarks->is_absent)
                                <select name="student_remarks[{{ "{$student->section_id}-{$remarks->subject_id}" }}][{{ $student->student_session_id }}][grades][{{ $remarks->exam_schedule_id }}]" class="grade-wrap {{ ($remarks->grade?->is_fail) ? 'border-red' : '' }}">
                              ...
                              @else
                                <span class="absent-wrap" style="color: {{ $remarks->attendance_status->color }};">{{ $remarks->attendance_status->name }}</span>
                              @endif
                            </td>
                          @break
                          @case('marks')
                            <td>
                              @if(!$remarks->is_absent)
                                <input type="number" name="student_remarks[{{ "{$student->section_id}-{$remarks->subject_id}" }}][{{ $student->student_session_id }}][marks][{{ $remarks->exam_schedule_id }}]" min="0" max="{{ $remarks->total_marks }}" class="marks-wrap" value="{{ $remarks->obtain_marks }}">
                              @else
                                <span class="absent-wrap" style="color: {{ $remarks->attendance_status->color }};">{{ $remarks->attendance_status->name }}</span>
                              @endif
                            </td>
                          ...
                          @case('category_grade')
                            <td>
                              @if(!$remarks->is_absent)
                                <select name="student_remarks[{{ "{$student->section_id}-{$remarks->subject_id}" }}][{{ $student->student_session_id }}][grading_categories][{{ $remarks->category_id }}]" class="grade-wrap">
                                  <option value="">Grade</option>
                                  @foreach($data['gradings'] as $grade)
                                    <option @selected($remarks->grade?->id == $grade->id) value="{{ $grade->id }}">{{ $grade->grade }}</option>
                                  @endforeach
                          ...
                          @case('category_marks')
                            <td>
                              @if(!$remarks->is_absent)
                                <input type="number" name="student_remarks[{{ "{$student->section_id}-{$remarks->subject_id}" }}][{{ $student->student_session_id }}][categories][{{ $remarks->category_id }}]" min="0" max="{{ $remarks->total_marks }}" class="marks-wrap" value="{{ $remarks->obtain_marks }}">
                              @else
                                <span class="absent-wrap" style="color: {{ $remarks->attendance_status->color }};">{{ $remarks->attendance_status->name }}</span>
                              @endif
                            </td>
