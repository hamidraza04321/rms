...
$(document).ready(function() {
	...
	//---------- ON CLICK SAVE ATTENDANCE STATUS ----------//
	$(document).on('click', '#btn-save-attendance-status', function(e) {
		e.preventDefault();
		removeErrorMessages();
		...
		if (flag) {
			...
			var form = $('#create-attendance-status-form');
	...
	//---------- ON CLICK UPDATE ATTENDANCE STATUS ----------//
	$(document).on('click', '#btn-update-attendance-status', function(e) {
		...
		if (flag) {
			...
			var form = $('#update-attendance-status-form');
			    url = form.attr('action');
			    formData = form.serialize();
	...
	//---------- ON CLICK DESTROY ATTENDANCE STATUS ----------//
	$(document).on('click', '.btn-destroy-attendance-status', function(e) {
		e.preventDefault();
		removeErrorMessages();
		 	...
		 	if (result.isConfirmed) {
				...
				self.addClass('disabled').html('<div class="spinner-border-sm"></div>');
		 		...
		 		$.ajax({
		 			...
		 			success: function(response) {
		 				if (response.status == true) {
		 					var table = $('#attendance-status-table').DataTable();
	...
	//---------- ON CLICK DELETE ATTENDANCE STATUS ----------//
	$(document).on('click', '.btn-delete-attendance-status', function(e) {
		e.preventDefault();
		removeErrorMessages();
		 	...
		 	if (result.isConfirmed) {
				...
				self.addClass('disabled').html('<div class="spinner-border-sm"></div>');
		 		...
		 		$.ajax({
		 			...
		 			success: function(response) {
		 				if (response.status == true) {
		 					var table = $('#attendance-status-trash-table').DataTable();
		 					table.row(self.parents('tr')).remove().draw();
		 					toastr.success(response.message);
		 				} else {
		 					message = errorMessage(response.message);
	...
	$(document).on('click', '.btn-restore-attendance-status', function(e) {
		e.preventDefault();
		removeErrorMessages();
