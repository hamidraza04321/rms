...
$(document).ready(function() {
	...
	//---------- ON CLICK SAVE SUBJECT ----------//
	$(document).on('click', '#btn-save-subject', function(e) {
		e.preventDefault();
		removeErrorMessages();
	...
	//---------- ON CLICK DESTROY SUBJECT ----------//
	$(document).on('click', '.btn-destroy-subject', function(e) {
		e.preventDefault();
		removeErrorMessages();
		 	...
		 	if (result.isConfirmed) {
				...
				self.addClass('disabled').html('<div class="spinner-border-sm"></div>');
		 		...
		 		$.ajax({
		 			...
		 			success: function(response) {
		 				if (response.status == true) {
		 					var table = $('#subject-trash-table').DataTable();
		 					table.row(self.parents('tr')).remove().draw();
		 					toastr.success(response.message);
		 				} else {
		 					message = errorMessage(response.message);
	...
	$(document).on('click', '.btn-restore-subject', function(e) {
		e.preventDefault();
		removeErrorMessages();
